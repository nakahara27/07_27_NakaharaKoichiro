...
<?php
	...
	// データ取得SQL作成
	$sql='SELECT * FROM todo_tbl WHERE is_eliminated=true ORDER BY deadline';
	...
	if($status==false) {
		...
		foreach ($ary_result as $asc_rec){
			...
			$created_at=substr($asc_rec["created_at"],0,16);//秒カット
			$updated_at=substr($asc_rec["updated_at"],0,16);//秒カット
			//
			$output.="<tr>";
			$output.="<td>{$deadline}</td>";
			$output.="<td>{$asc_rec["todo"]}</td>";
			$output.="<td>$created_at</td>";
			$output.="<td>$updated_at</td>";
			// edit deleteリンクを追加
...
<body>
	<fieldset>
		<legend>除外されたToDoリスト</legend>
		<a href="index.php">ToDoリストに戻る</a>
		<table>
			<thead>
				<tr>
					<th>締切日</th>
